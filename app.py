...
# Import new hourly predictor
try:
    from predict_disruption_ridership import DisruptionRidershipPredictor
    PREDICTOR_AVAILABLE = True
except ImportError:
    DisruptionRidershipPredictor = None
...
# Initialize new hourly ridership predictor
ridership_predictor = None
...
async def root():
    return {
        "message": "SwiftRoute API v2.1 - Enhanced with Hourly Ridership Prediction",
        "status": "operational",
        "hourly_predictor_available": ridership_predictor is not None,
        "endpoints": [
            "/predict_best_route", 
            "/forecast_ridership",
            "/predict_disruption_ridership",  # NEW
...
@app.post("/predict_disruption_ridership")
def predict_disruption_ridership(request: RidershipRequest):
    """NEW: Predict ridership at exact hour with detailed analysis"""
    if ridership_predictor is None:
