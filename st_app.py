...
st.set_page_config(page_title="SwiftRoute AI v2.1", layout="wide")
...
st.markdown(f"""
    <div style='position: relative; top: 0px; left: 0; right: 0;
        ...
        <div style='display: flex; align-items: center; gap: 24px;'>
            <img src='data:image/png;base64,{logo_base64}' width='120' style='border-radius: 12px; box-shadow: 0 0 8px #eee;' />
            <div>
                <h2 style='margin: 0; color: #1f4e79;'>AI Replacement Bus Launch Point Optimisation v2.1</h2>
                <p style='margin: 0; font-size: 19px; color: #444;'>Bus Replacement Optimization for Disrupted Routes</p>
            </div>
        </div>
    </div>
""", unsafe_allow_html=True)
...
if "api_status" not in st.session_state:
    st.session_state.api_status = "unknown"
# NEW: Add session state for detailed predictions
if "disrupted_ridership_details" not in st.session_state:
    st.session_state["disrupted_ridership_details"] = None
if "candidate_ridership_details" not in st.session_state:
    st.session_state["candidate_ridership_details"] = {}

# [Keep all your mappings]
depot_mapping = {
    "29": 0, "7": 1, "10": 2, "22": 3, "27": 4, "2": 5, "5": 6, "37": 7, "4": 8, "38": 9,
}
...
# [Keep disruption_places dictionary]
disruption_places = {
    "Putrajaya Sentral to Bandar Utama (PKNS Sports Complex)": {
        "lat": 3.0943772, "lng": 101.6009691, "route_no": "506", "depot": "5"
    },
...
# NEW: Enhanced ridership prediction function
def get_hourly_ridership_prediction(route_no, datetime_str):
    """Get detailed hourly ridership prediction"""
    try:
        payload = {
            "route_no": route_no,
            "disruption_datetime": datetime_str
        }
        response = requests.post(
            f"{API_URL}/predict_disruption_ridership",
            json=payload,
            timeout=30
        )
        ...
        st.error(f"Error predicting ridership for route {route_no}: {e}")
        return None

# Legacy forecast function (keep for backward compatibility)
def forecast_ridership_api(route_no_enc, depot_enc):
    """Forecast ridership with enhanced error handling"""
    payload = {
        "route_no_enc": route_no_enc,
...
# Main prediction button with ENHANCED hourly prediction
if st.sidebar.button("🚀 Forecast Ridership for Predicted Routes"):
    errors = []
    if disruption_route_no_enc == -1:
        errors.append(f"Route No '{disruption_route_no_orig}' not found in mapping.")
    ...
    if errors:
        ...
        with st.spinner("🔍 Analyzing disruption and forecasting optimal routes..."):
            ...
            try:
                ...
                st.sidebar.success(f"✅ Found {len(candidate_routes)} candidate routes")
                st.sidebar.success(f"🎯 Model Accuracy: {result_route.get('model_accuracy', 86):.1f}%")
                
                # NEW: Get detailed hourly predictions
                if today is not None:
                    disruption_time_str = today.strftime('%Y-%m-%d %H:%M')
                    
                    # Predict disrupted route with new system
                    st.sidebar.markdown("---")
                    st.sidebar.markdown(f"### 📊 Disrupted Route {disruption_route_no_orig}")
                    
                    disrupted_pred = get_hourly_ridership_prediction(
                        disruption_route_no_orig,
                        disruption_time_str
                    )
                    
                    if disrupted_pred and 'error' not in disrupted_pred:
                        st.session_state["disrupted_ridership_details"] = disrupted_pred
                        
                        col1, col2 = st.sidebar.columns(2)
                        with col1:
                            st.metric(
                                "Predicted Ridership",
                                f"{disrupted_pred['predicted_ridership']:.0f} pax"
                            )
                        with col2:
                            st.metric(
                                "Confidence",
                                f"{disrupted_pred['confidence_score']:.0%}"
                            )
                        
                        risk_level = disrupted_pred['risk_assessment']['level']
                        if risk_level == 'High':
                            st.sidebar.error(f"⚠️ {disrupted_pred['risk_assessment']['note']}")
                        elif risk_level == 'Medium':
                            st.sidebar.warning(f"⚠️ {disrupted_pred['risk_assessment']['note']}")
                        else:
                            st.sidebar.info(f"ℹ️ {disrupted_pred['risk_assessment']['note']}")
                    
                    # Predict candidate routes
                    st.sidebar.markdown("### 🚌 Candidate Routes Analysis")
                    
                    candidate_predictions = {}
                    progress_bar = st.sidebar.progress(0)
                    
                    for idx, route_no in enumerate(candidate_routes[:10]):  # Top 10
                        pred = get_hourly_ridership_prediction(route_no, disruption_time_str)
                        if pred and 'error' not in pred:
                            candidate_predictions[route_no] = pred
                        progress_bar.progress((idx + 1) / min(10, len(candidate_routes)))
                    
                    progress_bar.empty()
                    
                    st.session_state["candidate_ridership_details"] = candidate_predictions
                    ...
                    if candidate_predictions:
                        comparison_df = pd.DataFrame([
                            {
                                'Route': route,
                                'Ridership': f"{pred['predicted_ridership']:.0f}",
                                'Confidence': f"{pred['confidence_score']:.0%}",
                                'Risk': pred['risk_assessment']['level']
                            }
                            for route, pred in list(candidate_predictions.items())[:5]
                        ])
...
disrupted_details = st.session_state.get("disrupted_ridership_details")
candidate_details = st.session_state.get("candidate_ridership_details", {})

# Map visualization (keep as is)
depots = {
    "Depoh Sentul": (3.1880, 101.6900),
    "Depoh Batu Caves": (3.2375, 101.6815),
    "Depoh Melawati": (3.2200, 101.7150),
...
# Guided tour (keep as is)
guide_steps = [
    "1/6 - Welcome to SwiftRoute AI v2.1! Now with hourly ridership prediction. Click 'Next' to start.",
    "2/6 - The map shows current bus breakdowns and nearby depots for quick response.",
    "3/6 - Use sidebar controls to choose and simulate a breakdown scenario.",
    "4/6 - Click 'Forecast Ridership' to get AI predictions with 86% accuracy and real-time confidence scoring.",
    "5/6 - Charts show forecasted ridership with confidence intervals. Lower ridership = better rerouting option.",
    "6/6 - Done! Uncheck 'Show Guided Tour' in the sidebar to use the full system.",
]
...
# Enhanced visualization with detailed predictions
if candidate_details:
    st.markdown("### 📊 Detailed Ridership Analysis (Hourly Predictions)")
    
    # Create detailed comparison table
    comparison_data = []
    
    # Add disrupted route if available
    if disrupted_details:
        comparison_data.append({
            'Route': f"⚠️ {disrupted_details['route_no']} (Disrupted)",
            'Predicted': disrupted_details['predicted_ridership'],
            'Lower': disrupted_details['prediction_range']['lower_bound'],
            'Upper': disrupted_details['prediction_range']['upper_bound'],
            'Confidence': f"{disrupted_details['confidence_score']:.0%}",
            'Risk': disrupted_details['risk_assessment']['level'],
            'Reliability': disrupted_details['reliability']
        })
    
    # Add candidate routes
    for route, pred in candidate_details.items():
        comparison_data.append({
            'Route': route,
            'Predicted': pred['predicted_ridership'],
            'Lower': pred['prediction_range']['lower_bound'],
            'Upper': pred['prediction_range']['upper_bound'],
            'Confidence': f"{pred['confidence_score']:.0%}",
            'Risk': pred['risk_assessment']['level'],
    ...
    for idx, row in comp_df.iterrows():
        color = 'crimson' if '⚠️' in str(row['Route']) else 'steelblue' if row['Predicted'] < 20 else 'orange'
        ...
        fig.add_trace(go.Bar(
            ...
            name=row['Route'],
            marker_color=color,
            text=f"{row['Predicted']:.0f}",
            textposition='outside',
            showlegend=False,
            error_y=dict(
                type='data',
                symmetric=False,
                array=[row['Upper'] - row['Predicted']],
                arrayminus=[row['Predicted'] - row['Lower']],
                visible=True,
                color='gray',
                thickness=2,
                width=5
    ...
    fig.update_layout(
        title={
            'text': '📊 Hourly Ridership Predictions with 95% Confidence Intervals',
            'x': 0.5,
            'xanchor': 'center',
            'font': dict(family='Arial, sans-serif', size=24, color='darkblue')
        },
    ...
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("Routes Analyzed", len(comp_df))
    with col2:
        st.metric("Avg Ridership", f"{comp_df['Predicted'].mean():.1f}")
    with col3:
        st.metric("Lowest Ridership", f"{comp_df['Predicted'].min():.1f}")
    with col4:
        st.metric("Highest Ridership", f"{comp_df['Predicted'].max():.1f}")
    
    # Recommendations
    st.markdown("### 🎯 Top 5 Recommended Routes (Lowest Ridership)")
    top_5 = comp_df[~comp_df['Route'].str.contains('⚠️')].nsmallest(5, 'Predicted')
    st.dataframe(top_5[['Route', 'Predicted', 'Confidence', 'Risk', 'Reliability']], use_container_width=True, hide_index=True)
