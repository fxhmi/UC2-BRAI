...
# class DisruptionRidershipPredictor:
#     """Lightweight ridership predictor - Memory optimized for Render"""
    
#     def __init__(self, model_dir='models', data_dir='data'):
#         """Initialize with minimal memory"""
#         # Load models
#         self.model = joblib.load(os.path.join(model_dir, 'xgb_hourly_ridership_model.pkl'))
#         self.scaler = joblib.load(os.path.join(model_dir, 'xgb_hourly_ridership_scaler.pkl'))
#         self.route_encoder = joblib.load(os.path.join(model_dir, 'encoders/route_encoder.pkl'))
...
#         # Load JSON stats (small file!)
#         with open(os.path.join(data_dir, 'route_statistics.json'), 'r') as f:
#             self.route_stats = json.load(f)
    
#     def get_available_routes(self):
#         return sorted(list(self.route_stats.keys()))
    
#     def predict_at_disruption(self, route_no, disruption_datetime, depot=None):
#         route_no = str(route_no).strip()
        
#         if isinstance(disruption_datetime, str):
#             disruption_datetime = datetime.strptime(disruption_datetime, '%Y-%m-%d %H:%M')
        
#         if route_no not in self.route_stats:
#             return {'error': f'Route {route_no} not found'}
        
#         info = self.route_stats[route_no]
        
#         hour = disruption_datetime.hour
#         day_of_week = disruption_datetime.weekday()
#         month = disruption_datetime.month
#         week_of_year = disruption_datetime.isocalendar()[1]
#         day_of_year = disruption_datetime.timetuple().tm_yday
...
#         route_std = info['std']
#         route_max = info['max']
#         depot_val = depot if depot else info['depot']
        
#         route_hour_avg = info['hourly'].get(str(hour), route_avg)
#         route_dow_avg = info['dow'].get(str(day_of_week), route_avg)
...
#         confidence = 0.90 if prediction > 20 else 0.85 if prediction > 10 else 0.70
#         if is_peak_hour:
#             confidence = min(0.95, confidence + 0.05)
        
#         mae = 4.24
#         range_mult = 1.2 if prediction > 10 else 1.5
#         lower_bound = max(0, prediction - mae * range_mult)
#         upper_bound = min(235, prediction + mae * range_mult)
        
#         risk_level = "High" if prediction > 40 else "Medium" if prediction > 25 else "Low" if prediction > 10 else "Very Low"
...
#             'predicted_ridership': round(prediction, 1),
#             'confidence_score': round(confidence, 2),
#             'reliability': 'High' if confidence >= 0.85 else 'Good',
#             'prediction_range': {
#                 'lower_bound': round(lower_bound, 1),
#                 'upper_bound': round(upper_bound, 1)
#             },
#             'risk_assessment': {
#                 'level': risk_level,
#                 'note': f'Predicted demand: {prediction:.0f} passengers'
#             },
#             'conditions': {
#                 'hour': hour,
#                 'is_peak_hour': bool(is_peak_hour),
...
#         return {
#             'route_no': route_no,
#             'average_ridership': round(info['avg'], 1),
#             'max_ridership': round(info['max'], 1),
#             'depot': info['depot']
#         }
